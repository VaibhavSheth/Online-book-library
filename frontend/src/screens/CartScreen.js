...
const CartScreen = () => {
  ...
  const qtyLoc = useLocation().search
  const qty = qtyLoc ? Number(qtyLoc.split('=')[1]) : 1
  console.log(qty)
  ...
  const removeFromCartHandler = (id) =>{
    dispatch(removeFromCart(id)) //goes to cart action
  }
      ...
      <ListGroup variant='flush'>
        {cartItems.map(item =>(
          <ListGroup.Item key={item.product}>
            <Row>
              <Col md={2}>
                <Image src ={item.image} alt={item.title} fluid></Image>
              </Col>
              <Col md={3}>
